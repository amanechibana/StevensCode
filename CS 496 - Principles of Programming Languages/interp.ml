...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | Proj(e,id) -> (* accesses a member *)
    eval_expr e >>= fields_of_recordVal >>= fun fields ->
    (match (List.assoc id fields) with 
    | (mut,ref) -> if mut then (int_of_refVal ref) >>= fun r -> Store.deref g_store r >>= fun n -> return n else return ref)
  | SetField(e1,id,e2) -> 
    eval_expr e1 >>= fields_of_recordVal >>= fun fields -> 
    eval_expr e2 >>= fun new_val -> 
    (match (List.assoc id fields) with 
    | (mut,ref) -> if mut then (int_of_refVal ref) >>= fun r -> Store.set_ref g_store r new_val >>= fun _ -> return UnitVal else error "Not Mutable")
  | Unit -> return UnitVal
  | Debug(_e) ->
