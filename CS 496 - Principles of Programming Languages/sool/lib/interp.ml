...
let rec is_subclass =
  ...
  if (id1 = id2) then return @@ BoolVal (id1 = id2)
  else (match List.assoc_opt id2 class_env with
    | None -> error ("is_subclass: class "^id2^" not found")
    | _ -> (match List.assoc_opt id1 class_env with
              | None -> error ("is_subclass: class "^id1^" not found")
              | Some(_super,_,_) -> if(_super = "") then return @@ BoolVal (id1 = id2) else is_subclass _super id2 class_env))
...
and   
  ...
  | IsInstanceOf (e,id) -> 
    ...
    fun (id2, _) -> is_subclass id2 id !g_class_env
  (* Debug *)
  | Debug(_e) ->
