...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x!=nullptr){
            y=x;
            if(find(key) != end()){ //check duplicate and delete
                std::stringstream dupe_err;
                dupe_err << "Attempt to insert duplicate key '" << z->key() << "'.";
                delete z;
                throw tree_exception(dupe_err.str());
            } else if(z->key() < x->key()){
                x = x->left;
            } else{
                x = x->right;
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // TODO
        if (n == nullptr){
            delete n;               // deletes the current node if nullptr
            size_ -= 1;
        } else{
            if (n->left != nullptr){
                delete_tree(n->left); // deletes the left tree
            }
            if (n->right != nullptr){
                delete_tree(n->right); // deletes the right tree
            }
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        while(z->parent != nullptr && z->parent->color == RED ){
            ...
            } else{                                     // same but left and right swapped
    ...
    void left_rotate(Node<K, V> *x) {
        ...
        y->parent = x->parent;          // link the parents
        if(x->parent == nullptr){
            root_ = y;
    ...
    int height(Node<K, V> *node) const {
        if(node==nullptr){      // if empty tree, height -1
            return -1;
        } else{                 // increases the height count by 1 and takes the greater of the heights between right and left subtrees
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int left_diameter = diameter(node->left); //diameter of left
        int right_diameter = diameter(node->right); //diameter of right

        int diam_node = left_height + right_height + 2; // two diameters connected by node (1 node is 2 edge)
        int diam = std::max(left_diameter, right_diameter); 
        return std::max(diam_node,diam);;
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        if(level == 0){
            return 1;
        }       // recursively call the width of left and right subtrees
    ...
    size_t null_count(Node<K, V> *node) const {
        if(node==nullptr){
            return 1; // if null pointer is found return 1 
        } else{
            return null_count(node->right) + null_count(node->left); //if null pointer not found then continue search
        }
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        if(node == nullptr){
            return 0;
        } else{             // increments level by 1 for both sides
