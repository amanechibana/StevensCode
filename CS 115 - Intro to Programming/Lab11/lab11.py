...
def insertV2(L, i):
    ...
    number = L[i]
    L.pop(i)
    L.insert(search(L, i, number),number)
...
def letterCounts(fname):
    ...
    case letters the same.  Print the list9 of pairs (n,c) where c is a letter
    ...
    for line in file:
        for char in line:
            if letter(char) in dictionary:
                dictionary[letter(char)] += 1
            elif letter(char) == "non-letter":
                nonletterCount +=1
            else:
                dictionary[letter(char)] = 1
    ...
    for l2 in dictionary:
        result+=[(dictionary[l2],l2)]
    result.sort()
    result.reverse()
    
    return [(nonletterCount,"non-letter")]+result 
